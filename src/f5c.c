...
core_t* init_core(const char* bamfilename, const char* fastafile,
    ...
    //eventalign related
    core->mode = mode;
    if(mode==1 && EVENT_SUMMARY_FILE_PATH != NULL){
        core->event_summary_fp = fopen(EVENT_SUMMARY_FILE_PATH,"w");
        F_CHK(core->event_summary_fp,EVENT_SUMMARY_FILE_PATH);
    }
    ...
    for (i = 0; i < db->n_bam_rec; i++){
        if(!db->read_stat_flag[i]){
            ...
            if(core->mode==0) {
                ...
                emit_event_alignment_tsv(OUTPUT_FILE_POINTER,0,&(db->et[i]),core->model,db->scalings[i],*event_alignment_result, 1, 0, 0,
                              qname, contig);
            }
        }
        else{
